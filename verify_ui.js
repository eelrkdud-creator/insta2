...
(async () => {
    ...
    try {
        ...
        if (titleValues && titleValues.includes('인플루언서 게시물 업로드 시간 확인')) {
        ...
        try {
            ...
            if (resultText.includes('최초 업로드 시간 (KST)') || resultText.includes('수정 시간 (KST)') || resultText.includes('비공개') || resultText.includes('실패했습니다')) {
